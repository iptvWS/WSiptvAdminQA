...
{
    $sql = "SELECT ID, USUARIO, IDROKU, IP , PERMISOS FROM CSTCONTROL WHERE ID>5 ORDER BY ID";
   ...
   // <td > Permisos </td>
    ...
    $tabla.="<tr id='encabezado' style='border-width: 2px' id='fila0'>	<td nowrap> Sel </td><td nowrap> ID Usuario </td><td nowrap> Usuario </td><td nowrap> ID de dispositivo </td><td nowrap> IP de conexi&oacute;n </td><td nowrap> Dispositivo Habilitado </td></tr>";
    ...
    if( $rs )
    {
        ...
        if( pg_num_rows($rs) > 0 )
        {
            ...
            while ($row = pg_fetch_row($rs)) {
                ...
                $tabla.="<option value='S' ".($row[4]=="S"?"selected":"").">S</option><option value='N' ".($row[4]=="N"?"selected":"").">N</option>";
